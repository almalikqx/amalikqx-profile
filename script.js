const ICONS = {
  ...
  x: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M18.244 2H21l-6.522 7.45L22 22h-6.796l-4.766-6.23L4.8 22H2.044l7.006-7.99L2 2h6.914l4.31 5.71L18.244 2zm-2.388 18h1.878L7.006 4H5.017l10.839 16z"/></svg>`,
  tiktok: `<svg viewBox="0 0 24 24" fill="currentColor"><path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5.8 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1.84-.1z"/></svg>`,
  default: `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"></path><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"></path></svg>`
};

function initProfile() {
  document.getElementById('domName').innerText = CONFIG.name;
  document.getElementById('domUsername').innerText = CONFIG.username;
  document.getElementById('domBio').innerText = CONFIG.bio;
  document.getElementById('currentYear').innerText = new Date().getFullYear();
  
  document.getElementById('domProfile').src = CONFIG.profileUrl;
  document.getElementById('domCover').src = CONFIG.coverUrl;
  ...
  for (const [key, url] of Object.entries(CONFIG.socials)) {
    if (url && url.trim() !== "") {
      ...
      let handle = '@' + CONFIG.username;
      
      let nameLabel = key.charAt(0).toUpperCase() + key.slice(1);
      if (nameLabel === 'X') nameLabel = 'X (Twitter)';

      const iconSvg = ICONS[key] || ICONS.default;

      const linkEl = document.createElement('a');
      linkEl.className = 'social-link';
      linkEl.href = url; // Ini yang bikin kebuka url-nya
      linkEl.target = '_blank'; // Biar kebuka di tab baru
      linkEl.rel = 'noopener noreferrer';
      linkEl.innerHTML = `
        <span class="social-icon">${iconSvg}</span>
        <span class="social-info">
          <span class="social-name">${nameLabel}</span>
